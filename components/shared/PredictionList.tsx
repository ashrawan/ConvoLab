...
export default function PredictionList({
    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
            <div className="text-xs text-gray-500 mb-3 uppercase tracking-wide">Suggested Phrases</div>
            {predictions.map((pred, idx) => (
                <button
                    key={idx}
                    onClick={() => onPhraseSelect(pred.phrase)}
                    className="w-full text-left px-4 py-3 rounded-lg bg-white/5 hover:bg-violet-500/20 border border-white/10 hover:border-violet-500/30 transition group"
                >
                    {/* Main phrase */}
                    <div className="flex items-center gap-3">
                        <div className="flex-1">
                            <div className="text-base text-white font-medium group-hover:text-violet-300 transition">
                                {pred.phrase}
                            </div>
